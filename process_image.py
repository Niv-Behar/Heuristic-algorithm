...
def main(image_filename):
	...
	list_images = [[blank_image for i in range(20)] for i in range(20)]
	maze = [[0 for i in range(20)] for i in range(20)]
	for (x, y, window) in traversal.sliding_window(image, stepSize=30, windowSize=(winW, winH)):
		...
		if (any(i <= 120 for i in average_color)):
			maze[index[1]-1][index[0]-1] = 1
			occupied_grids.append(tuple(index))
		if (any(i <= 30 for i in average_color)):
